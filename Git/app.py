...
app.layout = html.Div([
    ...
    html.Div(id='help-text', style={'display': 'none'}, children=[
        html.H4('How to Use This Application'),
        html.H5('Upload your data files'),
        html.P(
            'Click the "Select Files" button to upload CSV or Excel files from your computer. Once selected, the file will be automatically uploaded, and you can view a preview of the dataset including the first and last few rows.'),
        html.H5('DataFrame Information'),
        html.P(
            'After uploading, click "Show DataFrame Info" to display information about the columns, data types, and the amount of data in your DataFrame. It helps you understand the structure of your data.'),
        html.H5('Missing Values Information'),
        html.P(
            'Use "Show Missing Values Info" to identify columns that contain missing values. This feature provides a quick summary of missing data counts per column, which is crucial for data cleaning and preparation.'),
        html.H5('Convert Columns to Categorical'),
        html.P(
            'If your dataset includes categorical data, use the "Convert to Categorical" button after selecting columns from the dropdown menu. This conversion is necessary for certain types of data analysis and machine learning model training.'),
        html.H5('Summary Statistics'),
        html.P(
            'Click "Show Summary Statistics" to get descriptive statistics for each column in your dataset. This includes count, mean, standard deviation, min, and max values, which are essential for initial data analysis.'),
        html.H5('Data Visualizations'),
        html.P(
            'Use "Show Visualizations" to generate histograms and bar charts for numeric and categorical data, respectively. Visualizations help in understanding data distribution and spotting patterns.'),
        html.H5('Data Resampling'),
        html.P(
            'The "Resample" button allows for balancing data, particularly useful in dealing with imbalanced datasets where some classes dominate others. This feature helps in improving model accuracy.'),
        html.H5('Model Training and Evaluation'),
        html.P(
            'After data preprocessing, use the "Evaluate" button to run machine learning models selected from the dropdown. This will provide performance metrics like accuracy, precision, recall, and F1-score for each model.'),
...
@app.callback(
    ...
    [State('stored-df', 'children')]
)
def generate_report(n_clicks, stored_df_json):
    if n_clicks > 0 and stored_df_json is not None:
        # Convert stored JSON to DataFrame
        df = pd.read_json(stored_df_json, orient='split')

        # Create a simple HTML representation of the DataFrame
        html_content = df.to_html()

        # Define path for saving the PDF
        pdf_path = 'path/to/save/report.pdf'

        # Convert HTML to PDF
        try:
            pdfkit.from_string(html_content, pdf_path)
            return html.Div('Report generated successfully. Check the file at {}'.format(pdf_path))
        except Exception as e:
            return html.Div('Failed to generate report: {}'.format(str(e)))

    return html.Div('Click the button to generate a report.')
